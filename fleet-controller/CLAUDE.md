...
| 19 | `detect_outbox_staleness` (tracker-event-board-pusher) | Compares a ticket's event outbox latest `seq` against each configured board's own cursor file (`.{tid}-cursor-{board_id}.json`, read directly via `jq` — never the locked cursor library, since detection only needs a read). Flags a `(ticket, board)` pair whose oldest unconsumed entry is older than `FLEET_OUTBOX_STALE_THRESHOLD_SECS`. Runs regardless of held/not-held — staleness is a property of the outbox/cursor files, not of a live process. **Hard-capped at severity 1, same rationale as `detect_human_hold`/`detect_observer_findings`/`detect_worker_api_errors`** — there is no process to kill, and in this phase no board mutation has been lost either (the shipped `linear` driver is a no-op for every event this phase covers), only delayed. Detection only — never drains, dispatches, or mutates anything; the accepted gap it makes observable (no fleetd running and the router killed externally) is documented in tracker-event-board-pusher's design.md | 0–1 (hard cap) |
...
- **Board pusher, wired but no-op** (tracker-event-board-pusher, Phase B2 of the tracker-decoupling programme): `fleetd/pusher.py` drains each ticket's event outbox (`ticket-auto-pipeline/lib/events.sh`'s `{tid}-outbox.jsonl`, Phase B1) against every board named in `FLEET_BOARD_DRIVERS`, dispatching unconsumed entries in `seq` order via subprocess calls to `lib/board-cursor.sh` and a driver's `apply` CLI — the same contract `skills/ticket-flow/outbox-drain.sh` (the router's own exit-time drain) uses, so board state after a drain never depends on which of the two drained it. The one shipped production driver (`lib/board-drivers/linear.sh`) is a **no-op for every event in this phase** — the ten hold/lifecycle facts B2 routes through it for the first time have zero existing Linear projection to reproduce, so there is nothing yet for a real mapping to keep byte-identical to. Cursor state is a flock-guarded flat file beside the outbox (`.{tid}-cursor-{board_id}.json`), never a `fleet-state.db` row — fleetd's store is fleetd-process-only, and the standalone drain must produce identical results with fleetd not running at all. Ships gated behind `FLEET_BOARD_PUSHER_ENABLE` (default `false`), same "new supervisor-loop logic against live tickets stays inert until proven" precedent `FLEET_PHASE_DISPATCH_ENABLE` set. `detect_outbox_staleness` (#19) makes the accepted "no fleetd, router killed externally" gap observable rather than silent.
...
| `FLEET_BOARD_PUSHER_ENABLE` | false | Must be `true` for `run_observe` to call `_board_pusher_pass` (`fleetd/pusher.py`) at all — same "new supervisor-loop logic ships inert until proven" precedent as `FLEET_PHASE_DISPATCH_ENABLE` (tracker-event-board-pusher design.md Decision 6). `skills/ticket-flow/outbox-drain.sh` (the router's own exit-time drain) is unaffected by this flag — it is a single-invocation script, not a standing daemon loop |
