...
- **The exporter's own id derivation is unconditional** (fixed for issue
  ...
  `derive_span_id_hex(run_id, phase, step, start_ts)` (bundled as
  ...
  `sha256("span:{run_id}:{phase}:{step}:{start_ts}")[:16]` — both lowercase
  hex, the W3C traceparent shape. `generation` was the fourth key input
  until #361's follow-up review found two live collisions: `generation` (as
  the pipeline log actually records it) is stamped once at a run's first
  spawn and frozen for every later preamble call, so retries within one run
  shared a span id; and `phase` alone conflated every step of a multi-step
  phase (PR-REVIEW's four steps share one `PHASE` field). `step` and
  `start_ts` (the bracket's own `|waiting|` timestamp, always distinct
  between two brackets of the same `(run_id, phase, step)` by the log's own
  bracket-uniqueness rule) fix both without plumbing anything new through
  the preamble/spawn chain — a `DerivedSpan` already carries both. `OtlpEmitter`
  installs a queued `IdGenerator` on its `TracerProvider`
  (`build_queued_id_generator`, task 7.6) and always queues the derived
  trace id before creating a ticket's root span, and the derived span id
  before creating a phase span, whenever `run_id` is known — regardless of
  `FLEET_TRACE_PROPAGATE_ENABLE`. This is what makes a resume, or a
  fleetd/exporter restart that re-tails a pipeline log from byte zero with
  an empty in-memory `_roots` cache, idempotent: the same run always derives
  the same (trace_id, span_id) pairs, so a replayed export updates the
  existing trace rather than opening a new root. **Known limitation**,
...
- **Propagating that context into the worker's own environment stays gated.**
  ...
  `|waiting|` line, so it has no `start_ts` to key on yet and passes `None`
  for it, deriving a value the exporter's later, real `start_ts`-keyed
  derivation will not reproduce. **Not yet settled**: whether a derived
  phase span — which reaches the backend *after* the children that attach
  beneath it, since the exporter is a tailing reader — actually reconciles
  into one trace at the real backend, span-id mismatch aside. That is phase
  4 (design.md Gate verdicts), a live end-to-end ticket run this change has
  not
