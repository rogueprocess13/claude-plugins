...
fleet-controller/
  ...
  fleetd/pusher.py                # Tracker event-board pusher (tracker-event-board-pusher) — drains ticket outboxes to board drivers
...
| `fleet-detect.sh` | 19 detection engines: `detect_phase_failures`, `detect_stalls`, `detect_zombies`, `detect_loops`, `detect_abandoned`, `detect_flow_failures`, `detect_auto_mode_blocks`, `detect_tool_errors`, `detect_planner_feedback`, `detect_blocked_by`, `detect_initiative_dispatch`, `detect_epic_branch_ready`, `detect_runaway_calls`, `detect_workspace_config`, `detect_human_hold`, `detect_observer_findings`, `detect_worker_api_errors`, `detect_stalled_approved_children`, `detect_outbox_staleness`. Aggregator: `fleet_detect_all` outputs JSON. Sourceable library — no `set -euo pipefail`. |
| `pusher.py` (fleetd) | `pusher_pass(log_dir, ...)` / `drain_ticket(tid, ...)` — tracker event-board pusher (tracker-event-board-pusher, Phase B2). Glob-discovers `*-outbox.jsonl` files under the pipeline log directory (never a registry) and, per ticket and per board in `FLEET_BOARD_DRIVERS`, drains unconsumed outbox entries in `seq` order via subprocess calls to `lib/board-cursor.sh` (`get`/`advance`) and the driver's own `apply` CLI — the identical contract `skills/ticket-flow/outbox-drain.sh` uses, so a ticket's board state after a drain never depends on which of the two drained it. Locking is Python's own `fcntl.flock` on the same lock-file path the bash cursor library locks (flock contention is per-inode, so this correctly excludes bash callers too). Wired into `Supervisor.run_observe` on its own cadence (`_board_pusher_pass`, `FLEET_BOARD_PUSHER_INTERVAL`), gated behind `FLEET_BOARD_PUSHER_ENABLE` (default `false`) — `run_observe` does not call it at all while the flag is unset. |
...
| 19 | `detect_outbox_staleness` (tracker-event-board-pusher; real projection since tracker-flow-projection-cutover) | Compares a ticket's event outbox latest `seq` against each configured board's own cursor file (`.{tid}-cursor-{board_id}.json`, read directly via `jq` — never the locked cursor library, since detection only needs a read). Flags a `(ticket, board)` pair whose oldest unconsumed entry is older than `FLEET_OUTBOX_STALE_THRESHOLD_SECS`. Also checks the ticket's pipeline log for a `META\|board-dead-letter` marker — a dead-lettered entry reports as a **lost** mutation, not a pending one. Runs regardless of held/not-held — staleness is a property of the outbox/cursor files, not of a live process. **Hard-capped at severity 1, same rationale as `detect_human_hold`/`detect_observer_findings`/`detect_worker_api_errors`** — there is no process to kill. Detection only — never drains, dispatches, or mutates anything; `BOARD_PROJECTION_STALLED` (raised by the pusher/`outbox-drain.sh` itself at dead-letter time, not by this detector) is the louder operator signal for the lost-mutation case | 0–1 (hard cap) |
...
- **The exporter's own id derivation is unconditional** (fixed for issue
  ...
  inflation). `fleetd/otel.py`'s `derive_trace_id_hex(run_id)` and
  `derive_span_id_hex(run_id, phase, step, occurrence)` (bundled as
  `derive_trace_context`) — pure functions, used by both the spawning path
  and this exporter for the *trace* id, so both sides compute the identical
  trace id with no shared state and no ordering requirement (TP1). A trace
  id is `sha256("trace:{run_id}")[:32]`; a phase's span id is
  `sha256("span:{run_id}:{phase}:{step}:{occurrence}")[:16]` — both
  lowercase hex, the W3C traceparent shape. `generation` was the fourth key
  input until #361's follow-up review found two live collisions:
  `generation` (as the pipeline log actually records it) is stamped once at
  a run's first spawn and frozen for every later preamble call, so retries
  within one run shared a span id; and `phase` alone conflated every step of
  a multi-step phase (PR-REVIEW's four steps share one `PHASE` field).
  `step` fixed the second outright. The first fix for the retry collision
  keyed on the bracket's own `|waiting|` timestamp instead, reasoning that
  two brackets of the same `(run_id, phase, step)` are never open at once —
  a second review round found that reasoning false: `phase_bracket_open`
  (`ticket-auto-pipeline/lib/spawn-helper.sh`) stamps at **second**
  resolution (`date -u +%Y-%m-%dT%H:%M:%SZ`, no sub-second component), so a
  fast, deterministic failure immediately followed by its retry's
  `|waiting|` line can land in the same UTC second and collide again. Fixed
  by `occurrence` instead — the 0-indexed count of `(phase, step)` brackets
  completed so far in this run, counted by `TicketTranslator.feed` as it
  reads the log in order and stamped onto each `DerivedSpan` at build time —
  which has no timing dependency at all, so it cannot collide regardless of
  how close together two brackets land, and stays deterministic on replay
  since a re-tailed log is always read in the same order. Nothing new is
  plumbed through the preamble/spawn chain either way — a `DerivedSpan`
  already carries `step`, and now `occurrence` too. `OtlpEmitter` installs a
  queued `IdGenerator` on its `TracerProvider` (`build_queued_id_generator`,
  task 7.6) and always queues the derived trace id before creating a
  ticket's root span, and the derived span id before creating a phase span,
  whenever `run_id` is known — regardless of `FLEET_TRACE_PROPAGATE_ENABLE`.
  This is what makes a resume, or a fleetd/exporter restart that re-tails a
  pipeline log from byte zero with an empty in-memory `_roots` cache,
  idempotent: the same run always derives the same (trace_id, span_id)
  pairs, so a replayed export updates the existing trace rather than opening
  a new root. **Known limitation**, matching design.md's Risks: if the root
  has to open under a provisional key (run id unknown at the moment the
  first phase bracket closes), its trace id was already randomly assigned
  before the run id became known and cannot be changed after creation — the
  derived trace id is adopted only when `run_id` is known at the moment the
  root is first created.
- **Propagating that context into the worker's own environment stays gated.**
  ...
  than opening an unrelated trace of its own. Its trace id matches the
  exporter's exactly (both depend only on `run_id`); its span id does not —
  this call runs *before* the worker it is spawning has written that phase's
  `|waiting|` line, so it has no sequential-read occurrence count to key on
  yet, and uses `attempt - 1` (the dispatch table's own 1-based retry
  counter, in scope for free at this call site; `0` when `attempt` is
  unknown) as an approximation rather than a constant. It will not generally
  equal the exporter's own occurrence count, which counts every prior span
  of that `(phase, step)`, not just retry attempts — so the span id this
  derives remains a distinct, deliberately approximate value from the one
  the exporter later derives for the same bracket. **Not yet settled**:
  whether a derived phase span — which reaches the backend *after* the
  children that attach beneath it, since the exporter is a tailing reader —
  actually reconciles into one trace at the real backend, span-id mismatch
  aside. That is phase 4 (design.md Gate verdicts), a live end-to-end ticket
  run this change has
  not
...
- **Board pusher, real projection** (tracker-event-board-pusher Phase B2; real mappings landed in tracker-flow-projection-cutover): `fleetd/pusher.py` drains each ticket's event outbox (`ticket-auto-pipeline/lib/events.sh`'s `{tid}-outbox.jsonl`) against every board named in `FLEET_BOARD_DRIVERS`, dispatching unconsumed entries in `seq` order via subprocess calls to `lib/board-cursor.sh` and a driver's `apply` CLI — the same contract `skills/ticket-flow/outbox-drain.sh` (the router's own exit-time drain) uses, so board state after a drain never depends on which of the two drained it. The one shipped production driver (`lib/board-drivers/linear.sh`) now writes real desired state — a column plus at most four human-signal labels — sourced from `workflow.json`'s `board_drivers.linear.events` table, preserving every label it does not own. A permanently-unprojectable event is recorded (`META|board-projection|fail`) and skipped; a repeatedly-failing one is dead-lettered after `FLEET_BOARD_MAX_ATTEMPTS` (default 5) consecutive attempts — `board-cursor.sh`'s `attempts` field tracks the streak, `board_cursor_advance` resets it on success — writing `META|board-dead-letter`, raising a `BOARD_PROJECTION_STALLED` Slack notification, and advancing past the entry so later events for that ticket keep projecting. Cursor state is a flock-guarded flat file beside the outbox (`.{tid}-cursor-{board_id}.json`), never a `fleet-state.db` row — fleetd's store is fleetd-process-only, and the standalone drain must produce identical results with fleetd not running at all. `FLEET_BOARD_PUSHER_ENABLE` stays `false` by default even now — see README.md's "Migrating to board projection" for the mandatory three-step order (deploy → `board-cursor-fastforward.sh` → flip the flag), since every existing host's cursors sit at 0 and enabling projection against them would replay full history. `detect_outbox_staleness` (#19) reads the dead-letter marker to distinguish a lost mutation from a merely-delayed one.
...
| `FLEET_ORPHAN_RESTART_GRACE_SECS` | 60 | GitHub #364. `_count_restarts` (`fleet-intervene.sh`) exempts a `META|fleet-restart` marker from the `FLEET_MAX_RESTARTS` cap when its window shows `META|orphan-reaped` evidence, no phase terminal, and the wall-clock time to the next restart marker (or now) is within this many seconds — an attempt whose whole life was spent reaping a leftover orphan pinger/watchdog and crashing again immediately, not a genuine hang. Two orders of magnitude below `FLEET_STALL_WARN_SECS`/`FLEET_STALL_RESTART_SECS`, so a real hang (which runs for the many minutes those thresholds require before fleet intervenes) is never exempted |
...
| `FLEET_BOARD_PUSHER_ENABLE` | false | Must be `true` for `run_observe` to call `_board_pusher_pass` (`fleetd/pusher.py`) at all. Stays `false` by default even after tracker-flow-projection-cutover gave the board driver a real projection — flipping it on a host whose outbox cursors have never been fast-forwarded replays every ticket's full transition history. See README.md's "Migrating to board projection" for the required deploy → fast-forward → enable order. `skills/ticket-flow/outbox-drain.sh` (the router's own exit-time drain) is unaffected by this flag — it is a single-invocation script, not a standing daemon loop |
| `FLEET_BOARD_PUSHER_INTERVAL` | 300 | Seconds between board-pusher cycles when enabled, `is_due()`-gated exactly like `FLEET_GATE_RECONCILE_INTERVAL` — defaults to the same value since both are periodic passes over ticket state with no sub-minute-latency requirement |
| `FLEET_BOARD_DRIVERS` | linear | Comma-separated board driver names dispatched to by both `fleetd/pusher.py` and `outbox-drain.sh`. A driver script present on disk but not named here is never invoked — configuration is explicit, not disk-discovery |
| `FLEET_OUTBOX_STALE_THRESHOLD_SECS` | 3600 | Age of a `(ticket, board)` pair's oldest unconsumed outbox entry above which `detect_outbox_staleness` reports WARN |
...
| `EPIC_BRANCH_PUSH_VERIFY` | false | Read by `ticket-auto-pipeline/lib/epic-branch.sh`. `true` forces the repo's pre-push hook to run on the epic-branch creation push. The default skips it (`git push --no-verify`) because that ref is asserted identical to `origin/<base>` — zero new commits, nothing for a hook to check — while a repo-local pre-push hook runs its preflight against whatever branch the shared `REPOS_ROOT` clone happens to have checked out, so an unrelated stale checkout could fail the push and gate-stop the whole initiative (GitHub #381). The skip is conditional on the zero-commit assertion, so implement-agent commit pushes are never affected |
