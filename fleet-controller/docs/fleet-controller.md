...
7. **Summary**: last stdout line is `fleet_dispatch: resumed N | dead-lettered N | blocked N | enqueued N ticket(s) for {epic}` (dry-run: `[DRY-RUN] would resume N | blocked N | would enqueue N ...`); `POST /dispatch` parses the per-tid lines into `resumed`/`blocked` arrays alongside `queued`, and scans both streams for gate-stop markers (`EPIC_BRANCH_UNAVAILABLE`, `epic-branch: failed to push`) — a match sets the response's `gate_stop` key and replaces `message` with that line, since gate-stops return rc 0 and would otherwise read as a clean no-op
