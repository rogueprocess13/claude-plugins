...
- `adr-gate` — reusable architectural governance gate; invoked by any phase via the shared preamble § 8, and by `ticket-planner`, as a plain skill invocation with no cross-plugin bash dependency
...
| `flow.sh` (in skills/ticket-flow/) | State machine executor. Reads `skills/ticket-flow/state-machine.json`. Handles state transitions, label add/remove, assignee changes. Idempotency: computes desired end state, skips if no change. Post-trigger assertions: re-fetches issue, exits 7 on mismatch. Verdict gate (issue #368): a trigger declaring `"verdict_gate": true` (`pr-review-pass-done`, `pr-review-pass-uat`, `uat-pass`) is refused (exit 11) when `lib/verifier-result.sh`'s `verifier_latest_verdict` finds a trailing FAIL/BLOCK for any `(verifier, phase)` pair with no later PASS/WARN — `--override <reason>` forces it through and logs `META|verdict-override`. |
...
| `verdict-recompute.sh` | `verdict_recompute --phase VERIFY --return-file <path> --ticket-dir <path> [--log-file <path>]`. RLVR verdict-recompute — the `phase-result` claim's counterpart. Reuses `phase-result-parse.sh`'s `parse_phase_result` (sourced, log-free) to obtain the claim from the same captured-return file, and `return-completeness-check.sh`'s `_count_checklist_boxes` (sourced) to count `- [x]`/`- [ ]` in `verify-session.md`'s `## Step trace` section — never a second checkbox parser. The verified verdict never trusts the claim: `verify-session.md` must exist and postdate the phase's own `\|VERIFY\|verify\|waiting\|` bracket-open line, or `verified_met=0` with `evidence_state` recording `missing`/`stale`. Appends `META\|claim-delta\|info\|{json}` — `claimed_*`, `verified_*`, `criteria_total`, `delta`, `direction` (`aligned`\|`optimistic`\|`pessimistic`\|`unknown`), `evidence`, `evidence_state`. VERIFY only; IMPLEMENT and PR-REVIEW recompute rules are deferred. Exit-code idiom follows `return-completeness-check.sh`, not its own claim parser: 0 aligned, 1 mismatch (a normal outcome, not an error), 2 script could not run. Observe-only — see [pipeline-log-format.md](pipeline-log-format.md#claim-delta-entries-rlvr-verdict-recompute). Sourceable lib and standalone CLI. |
...
| `wiki-check.sh` | Deterministic freshness/completeness lint for `WIKI_ROOT` (wiki-cross-repo-knowledge-layer). Per file: line count, frontmatter completeness (`verified_at`, `verified_against: {repo, sha}`, `stale_after`, `verified: unverified\|machine-verified\|human-reviewed`), broken `related:` links, backticked class names unresolved against any repo under `--repos-root` (best-effort, capped, dedup'd), and a `fresh`/`stale`/`decayed` classification derived from age-vs-`stale_after` and one `git rev-list --count` per file since `verified_against.sha` — never a per-line git call. `--file <path>` checks one file, `--changed-only` scopes to `git status --porcelain` in `WIKI_ROOT`'s own repo. Emits `WIKI_CHECK\|...` lines plus a `WIKI_CHECK_SUMMARY` line; exit 0 clean, 1 issues found, 2 usage error. Wired into `wiki-maintenance` (before its `WIKI_ROOT` commit) and `ticket-appraise` Step 3a (a `decayed` wiki file is demoted the same way a decayed prescan doc is — verify before trusting). |
...
| `openspec-tracking-check.sh` | Durability guard for openspec plan artifacts (issue #363, OPENSPEC_ARTIFACTS_UNTRACKED). `commit <change-dir> <TICKET-ID>` force-adds and commits a change dir into the tickets repo — idempotent (`git diff --cached --quiet` no-op check), past any blanket `openspec/` ignore rule that predates this requirement; refuses (exit 2) any `<change-dir>` not under `openspec/changes/` and always runs against CWD (no `--root` — meant to run immediately after the artifact is written, before any worktree `cd`). `assert <TICKET-ID> [--expect] [--root <path>]` reports one of `tracked\|partial\|untracked\|gitignored\|missing\|not-applicable` for a ticket's change dir; `partial` covers both "some files untracked" and a *tracked* file carrying an uncommitted modification (checked via `git diff --quiet`/`git diff --cached --quiet` on the dir, not just the `??`/`!!` porcelain prefixes `git status` reports for genuinely untracked/ignored paths — a modified-but-tracked file shows as ` M` there, which `tracked` must never silently pass). Without `--expect` a missing dir is `not-applicable` (simple-fix ticket), with it (complex ticket) it's `missing` and fails. `--root` lets a caller resolve against an explicit tickets-repo path instead of trusting ambient CWD — needed once a caller has `cd`'d elsewhere (e.g. `ticket-implement` after its own worktree `cd`). `audit [--root <path>]` sweeps every `openspec/changes/*/` dir and emits `OPENSPEC_TRACK\|...` lines plus an `OPENSPEC_TRACK_SUMMARY` line — the one-off detector for pre-existing untracked artifacts. Exit 0 clean, 1 issue(s), 2 usage/git error. Deliberately scoped to the tickets repo only — never a code repo, whose own `openspec/` ignore (if any) is a separate, unrelated concern. `ticket-appraise-exec` Step 3.4 calls `commit` right after the coherence gate passes for a complex ticket; `ticket-implement` captures `$IMPLEMENT_TICKETS_ROOT="$(pwd)"` as its first action (before Step 4's worktree `cd`, which nothing downstream reverses) and Step 5.5 calls `assert ... --root "$IMPLEMENT_TICKETS_ROOT"` as a close-out backstop (warns via `gate-warn`, never gate-stops — losing design rationale is a durability problem, not a correctness one). |
| `adr-store.sh` | `adr_write`, `adr_accept` (refuses invocation from an agent context — `CLAUDE_CODE_SESSION_ID` set — exit 4), `adr_supersede`, `adr_query_by_component`. Durable ADR store under `{WIKI_ROOT}/decisions/`. Gap-tolerant `NNNN-` numbering, per-source (`initiative`/`ticket`/`manual`) idempotency, `flock`-guarded atomic writes, wholesale index regeneration (`decisions/index.md`) on every write — never a hand-maintained row. Also sourced by `adr-check.sh`, `wiki-bootstrap.sh`, and `wiki-verify.sh` for shared frontmatter parsing (`_adr_extract_frontmatter`, `_adr_strip_frontmatter`) — one parser, not three. CLI and sourceable lib; distinct exit code per failure mode (adr-governance-gate). |
| `adr-check.sh` | Deterministic validator for the ADR store — every check in the adr-validation spec: schema, id format/uniqueness, status validity and transition validity (git-history based, detects an in-place edit to an Accepted ADR), `supersedes` target validity and reciprocity, required sections non-empty, new-ADRs-are-`proposed`, and the hedging-modal check on `## Decision` (bans `should`/`would`/`may`/`might`/`could`). `--file <path>` (single-file mode) or whole-store mode; `ADR_CHECK\|...` finding lines plus an `ADR_CHECK_SUMMARY` line; exit 0 clean, 1 violations, 2 usage error — same grammar as `wiki-check.sh` (adr-governance-gate). |
| `wiki-bootstrap.sh` | `wiki_bootstrap` — existence-gate scaffolding for `index.md`, `decisions/index.md`, and `glossary.md` from fixed templates; no-op when `WIKI_ROOT` is unset, no-op per artifact already present. `index.md`'s Lookup by Topic/Service tables match the shape `prescan-route.sh` already parses. `wiki_term_drift_check` — reusable scan for a glossary entry's `Avoid:` synonyms across wiki files and ADRs, reused by `wiki-verify.sh` and the `adr-gate` skill before drafting ADR prose. CLI (`bootstrap`\|`term-drift` subcommands) and sourceable lib (adr-governance-gate). |
| `adr-gate-parse.sh` | `parse_adr_gate_result` — deterministic parser for the `=== ADR_GATE_RESULT ===` block, adapted from `human-hold-parse.sh`. Tolerant at transport (whitespace, CRLF, field order, unknown fields), strict at contract (required fields, the closed `ADR_VERDICT` enum, `CONFLICT`'s mandatory `ADR_ID`/`CONFLICT` pair, closing marker). Unlike `human-hold-parse.sh`, an absent block is not a normal outcome — the gate is required to emit one on every invocation, so a missing block is a parser failure like any other. Writes `META\|adr-gate\|info\|{json}`. Exit 0 valid, 1 no usable verdict (still logged), 2 parser could not run (adr-governance-gate). |
...
| `run-identity.sh` | `run_identity_current LOG_FILE` (open run's `run_id`, or empty), `run_identity_stamp TID LOG_FILE [--new]` (writes `META|run-id` + `META|version`, no-op if a run is already open and `--new` wasn't passed), `run_identity_ticket_meta TID LOG_FILE` (writes `META|ticket-meta` once per ticket via `get_issue`, requires `LINEAR_API_KEY`, fails soft). Single writer for these three META keys — called from both `ticket-preamble.sh` (fleetd path, no `--new`) and `skills/ticket-auto/SKILL.md` Step 0.6 (manual router, with `--new` — one router process is one run). CLI: `run-identity.sh stamp TID LOG_FILE [--new]`. Adopts a `TICKET_RUN_ID` env var when opening a new run and it conforms to the `{TID}-{ISO}-{pid}` shape (run-identity-handoff, langfuse-evidence-layer) — fleetd mints or reuses this value *before* the spawn (`fleetd/supervisor.py`'s `_open_run_id`) and hands it down, because the spawn environment and every trace identifier derived from run identity have to exist before this file's own stamp would otherwise mint one. An empty or malformed value is refused, not propagated: minting falls back to the unchanged local path. |
| `exit-path.sh` | `_derive_exit_path` (extracted verbatim from `pipeline-postmortem.sh`, sourced by it — SC1, langfuse-evidence-layer), `derive_failure_class LOG_FILE` / `derive_failure_phase LOG_FILE` (run-failure-classification — a closed, ten-value failure vocabulary and its phase, derived deterministically from evidence the pipeline already writes: `held:`/gate-stop/router-error/fleet-kill outcome shapes, `META|verifier-result`, `META|phase-inspector`, `META|worker-api-error`, `META|gate-warn`. No model invocation, no new log line. Gate-stop codes are matched by prefix, not exact string — real evidence carries trailing detail text after the bare code (e.g. `ADVERSARIAL_BLOCKED — <description>`) that an exact match silently misses. A `RETURN_INCOMPLETE` gate-warn only counts when the run did not go on to complete — a resolved warning from an earlier retry must not override a clean finish. CLI: `exit-path.sh class\|phase LOG_FILE`. Replayed read-only against 53 real archived pipeline logs before shipping (design.md, task 10.10): 0 landed in the generic `agent_failure` fallback. |
...
| `gitnexus-preflight.sh` | `gitnexus_verify_branch <repo_git_dir> <indexed_commit> <expected_ref> [max_commits_behind]`, `gitnexus_check_result_subset <returned_files_list> <known_files_list>`. Deterministic GitNexus pre-flight verification (GITNEXUS_PREFLIGHT_BRANCH_UNVERIFIED, issue #359). `list_repos`' own `staleness.commitsBehind` is computed against whatever branch the *indexed clone* currently has checked out, not against the branch a caller cares about, so reachability + `commitsBehind == 0` can both read healthy while the index sits on a wholly unrelated branch — `gitnexus_verify_branch` closes that gap with a pure git ancestor/distance check (`ok <n>` / `wrong-branch` / `ahead <n>` — same lineage but the wrong direction, e.g. a rebase/force-push, functionally identical to `wrong-branch` but distinguished for operators grepping the heartbeat log / `stale <n>` / `unresolvable <reason>`; exit 0 verified, 1 unverified, 2 usage error — both 1 and 2 mean the caller must fall back). `gitnexus_check_result_subset` sanity-checks a `detect_changes` result set against the real diff's file list, catching a stale-but-ancestor index that still names files outside the actual change. Callers make the `list_repos`/`detect_changes` MCP calls themselves and pass results in — no MCP call happens inside this lib. Consumed by `ticket-pr-review` Step 4.5 and `ticket-implement` Step 5's pre-push check. |
| `verify-worktree.sh` | `verify_worktree_path <repo_slug> <TICKET_ID>`, `ensure_verify_worktree <TICKET_ID> <repo_path> <branch>`, `release_verify_worktree <TICKET_ID>`, `verify_worktree_gc`. `ticket-verify`'s own isolated-worktree layer — deliberately separate from `worktree.sh` (`REPOS_ROOT/.verify-worktrees/{repo}/{TICKET_ID}`, repo before ticket, no `<base>` since verify only ever checks out a branch implement already pushed). Unlike `ensure_worktree`, reuse always fetches and hard-resets to `origin/<branch>` — a verify run must see the branch's latest pushed commits, not whatever an earlier attempt left checked out — and touches the worktree root on every successful call regardless of what git itself did or didn't touch (a linked worktree's own directory mtime isn't reliably advanced by `reset`/`clean`, since git's bookkeeping for it lives under the main repo, not the worktree root). `verify_worktree_gc` is a TTL sweep (`VERIFY_WORKTREE_TTL_HOURS`, default 24h), mirroring `hooks/tmp-sweep.sh`'s age-based approach but invoked inline from `ticket-verify` Step 1.6a2 rather than as a separate hook, and skips its entire sweep whenever `verify-lock.sh`'s lock is currently held by anyone (lazily sourced if not already present) — the lock is one global flight, so there's no way to tell from here which ticket's worktree a live holder might be serving files from. SKILL.md deliberately runs GC (1.6a2, never touches this run's own worktree) before the lock (1.6a3), and `ensure_verify_worktree` itself (1.6a4, destructive on reuse) only after — a second `ticket-verify` invocation for the same ticket (the "zombie steps" sharp edge below) must never reset a worktree a still-live first invocation's app stack is serving. |
| `verify-lock.sh` | `verify_lock_acquire <TICKET_ID> [timeout_secs]`, `verify_lock_release`, `verify_lock_status`. Single-flight `flock` mutex serializing `ticket-verify`'s local app stack (hardcoded, non-namespaced ports) so only one verify run's stack is ever up at a time. Because `ticket-verify`'s SKILL.md runs as many independently-invoked bash blocks rather than one continuous shell, the lock can't be held via a plain `exec N>file; flock N` in the caller's own process — it's instead held by a small detached companion process (mirrors `spawn-helper.sh`'s `spawn_watchdog_start`: `( ... ) & disown`, a stop-file for cooperative release, a bounded max-hold time as the crash backstop). A dead holder needs no staleness check — the kernel drops the flock the instant that process's fd closes, for any reason. `VERIFY_LOCK_TIMEOUT_SECS` (default 2400s) bounds how long an acquirer waits; `VERIFY_LOCK_MAX_HOLD_SECS` (default 3600s) bounds how long an unreleased lock can wedge things open (e.g. a killed agent). `verify_lock_release` is identity-safe: it snapshots which pid it intends to release once at the start (before touching anything) and only ever acts on the token/info/stop files while their content still matches that snapshot — release legitimately overlaps with a brand-new holder winning the lock within milliseconds of the old one exiting, and an existence-only check can't tell "still my holder, just slow" from "somebody else already won," which would otherwise destroy the new holder's live bookkeeping on timeout. |
...
- **Safety gates**: 18 structural gate-stop codes (EXEC_NO_ARTIFACT, COMPLEXITY_ARTIFACT_MISMATCH, CRITIQUE_SCORE_MISSING, CRITIQUE_BLOCKED, CRITIQUE_SCORE_IMPLAUSIBLE, ZERO_AC, BUG_NO_REPRO, NO_TEMPLATE_FOR_TYPE, PLANNED_BODY_INCOMPLETE, APPROVAL_REVOKED, VERIFY_EXHAUSTED, PR_REVIEW_VERDICT_UNPARSEABLE, PR_FEEDBACK_EXHAUSTED, BRANCH_DIRECTIVE_INVALID, CODE_REVIEW_EXHAUSTED, RECONCILE_EXHAUSTED, ADR_CONFLICT, VERDICT_GATE_BLOCKED). Violations emit `|META|gate-stop|fail|<CODE>`. `VERDICT_GATE_BLOCKED` (issue #368) is agent-emitted, not router-detected: `ticket-verify`'s `uat-pass` call and `ticket-pr-review`'s `pr-review-pass-done`/`pr-review-pass-uat` call each check for `flow.sh` exit `11` specifically (the verdict gate's refusal code — see the `flow.sh` row above) and, on that exit code alone, skip the normal PASS comment/PHASE_RESULT `PASS`/merge and instead emit this code with `PHASE_RESULT: VERDICT: BLOCK` — a gate block must never look identical to a real completion in the log or in Linear, which is the inverted form of the bug #368 itself fixed.
...
- **Verdict recompute** (rlvr-verdict-recompute): `META|phase-result` is a self-report; nothing in the pipeline previously compared it against observable state. `lib/verdict-recompute.sh` runs immediately after `phase-result-parse.sh` on the VERIFY path, deriving `verified_verdict` from `verify-session.md`'s `## Step trace` checkbox counts — independent of, and never trusting, the claim — and appends `META|claim-delta|info|{json}` recording both sides plus a `direction` (`aligned`/`optimistic`/`pessimistic`/`unknown`). Split out of `rlvr-phase-result-contract` because the recompute half is RLVR measurement on its own timeline, depending only on the parser existing, not the reverse. Observe-only in this increment: no gate-stop, no routing change, and the claim is never written through `write_verifier_result` (a claimed-PASS beside a verified-FAIL would trip the flaky-tests/verdict-disagreement detection patterns on a channel that isn't a verifier result). IMPLEMENT and PR-REVIEW recompute rules, and any `direction`-driven routing (`gate-warn` reader or a `CLAIM_DRIFT` gate-stop), are explicitly deferred until the real-run distribution is reviewed — see [pipeline-log-format.md](pipeline-log-format.md#claim-delta-entries-rlvr-verdict-recompute).
...
- **Worktree isolation is files-only**: Per-ticket git worktrees isolate the working tree (branches, files, index). They do NOT isolate ports, databases, caches, or seeded test state. `ticket-verify` (0.47.0+) resolves its own isolated worktree per affected repo (`lib/verify-worktree.sh`, `REPOS_ROOT/.verify-worktrees/{repo}/{TICKET_ID}` — a separate tree from `ticket-implement`'s, since verify must never read a directory implement might still be writing to) and serializes the app stack itself with a single-flight `flock` (`lib/verify-lock.sh`) so only one verify's app stack is ever up against `LOCAL_URL` at a time. That closes the git-branch race and the concurrent-run race, but is deliberately *not* per-run port namespacing — a verify run still can't run in parallel with another against the same `LOCAL_URL`; it queues instead. True concurrent port/database/cache isolation remains unimplemented.
