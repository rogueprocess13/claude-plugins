...
| `openspec-tracking-check.sh` | Durability guard for openspec plan artifacts (issue #363, OPENSPEC_ARTIFACTS_UNTRACKED). `commit <change-dir> <TICKET-ID>` force-adds and commits a change dir into the tickets repo — idempotent (`git diff --cached --quiet` no-op check), past any blanket `openspec/` ignore rule that predates this requirement. `assert <TICKET-ID> [--expect]` reports one of `tracked\|partial\|untracked\|gitignored\|missing\|not-applicable` for a ticket's change dir; without `--expect` a missing dir is `not-applicable` (simple-fix ticket), with it (complex ticket) it's `missing` and fails. `audit [--root <path>]` sweeps every `openspec/changes/*/` dir and emits `OPENSPEC_TRACK\|...` lines plus an `OPENSPEC_TRACK_SUMMARY` line — the one-off detector for pre-existing untracked artifacts. Exit 0 clean, 1 issue(s), 2 usage/git error. Deliberately scoped to the tickets repo only (the CWD every subcommand runs `git` against) — never a code repo, whose own `openspec/` ignore (if any) is a separate, unrelated concern. `ticket-appraise-exec` Step 3.4 calls `commit` right after the coherence gate passes for a complex ticket; `ticket-implement` Step 5.5 calls `assert` as a close-out backstop (warns via `gate-warn`, never gate-stops — losing design rationale is a durability problem, not a correctness one). |
