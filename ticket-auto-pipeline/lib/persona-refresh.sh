...
cmd_bump() {
  ...
  # Validate: version must be a positive integer (defense against tampered YAML)
  [[ "$current" =~ ^[0-9]+$ ]] || die "invalid version in $file: '$current'"

  local new=$((current + 1))

  # Update version (safe: current/new are both validated integers)
  sed -i "s/^version: $current$/version: $new/" "$file"

  # Update last-reviewed (safe: today is generated by date -u +%Y-%m-%d)
