...
ticket-auto-pipeline/
  ...
  skills/ticket-flow/workflow.json        # Linear state/label transitions + event vocabulary (canonical)
...
**Support skills**: `ticket-critique`, `ticket-detect-resume`, `ticket-document`, `ticket-overseer`, `ticket-batch-appraise`, `ticket-batch-verify`, `ticket-reproduce`, `ticket-gate-reconcile`, `ticket-prescan`, `wiki-maintenance`, `nav-hints`, `app-knowledge`, `adr-gate`.
...
**`flow.sh` performs no tracker I/O** (tracker-flow-projection-cutover, Change 2 of the tracker-decoupling authority-flip programme). It is a local state machine: its inputs are `workflow.json` and the ticket's local manifest, its outputs are the manifest (`stage`, `flags`, `rev`, `pending_event`) and exactly one outbox event per invocation. It never fetches the issue, never fetches the team, never resolves a label or state id, and never mutates Linear directly. Skills never call Linear mutation endpoints directly either — the tracker receives its column and label projection later, asynchronously, from a board driver (`lib/board-drivers/linear.sh`) draining the outbox, never from `flow.sh` itself. `flow.sh` still owns: the trigger's `from`/precondition checks (resolved from the manifest, warn-only on mismatch), idempotency (the manifest write is skipped when the computed `(stage, flags)` already match, though rev/idem still advance every invocation), and generation fence checks (`{tid}-fence` — rejects mutations from superseded generations, gated behind `FLEET_FENCE_ENFORCE=true`). The state machine definition lives in `skills/ticket-flow/workflow.json` — `flow.sh` reads it, so changes to the JSON take effect without script changes. The same file carries a `vocabulary` section (the canonical, board-agnostic event catalog `lib/events.sh`'s `emit_event` validates against — see "Event outbox" below) and a `board_drivers` object: the one sanctioned, namespaced place a board's own column/label model may appear in this file.

**Event outbox** (tracker-event-vocabulary-and-emitter Phase B1; real projection landed in tracker-flow-projection-cutover): `ticket-auto-pipeline/lib/events.sh` exposes `emit_event [--idem KEY] <tid> <event> <json>` — the single entry point for writing a fact to a ticket's append-only outbox (`{tid}-outbox.jsonl`, JSON Lines, one record per line: `{seq, tid, ts, gen, event, data, from_hint, idem}`). Independent of `flow.sh` on purpose — gate holds and human holds write directly to the pipeline log and never call `flow.sh`, so emission cannot live inside it (`flow.sh`'s own idempotency rule would swallow a hold's emission, which changes no manifest state, even if it did). `--idem` makes a repeated emission with the same key a no-op (checked and appended under the same per-ticket lock), which is what lets `flow.sh` guarantee exactly one event per invocation across a crash on either side of the emit — it supplies `{tid}:{rev}` as the key, `rev` being the manifest's own monotonic transition counter. Every event-producing site — `flow.sh`-driven transitions (via each trigger's own `emits` declaration in `workflow.json`), gate holds/releases, human holds/releases, PR open/merge, verify retries, and fleet-initiated kills — calls `emit_event` directly or indirectly. The projection side is no longer inert: `board_drivers.linear.events` maps every vocabulary event to a column/label/assignee entry (or an explicit `null` for "projects nothing"), and `lib/board-drivers/linear.sh`'s `apply` writes desired state — column plus at most four human-signal labels (`needs-info`, `needs-adr`, `rejected`, `reviewed`) — never reading anything back for a decision. `skills/ticket-flow/outbox-drain.sh` runs at every `flow.sh` exit, unconditionally, so a person's own command projects immediately rather than waiting for `fleetd`'s periodic pusher pass. See `openspec/changes/tracker-flow-projection-cutover/` (or its archived spec once promoted) for the full design.
...
- **Complexity gating**: Simple tickets auto-approve in `auto`/`semi-auto` mode. Complex tickets always gate (require human approval via `/ticket-approve`, recorded as a local manifest fact — not a Linear label; see `tracker-approval-by-script`). The `manual` mode gates everything.
- **Phase context file**: Before each agent spawn, the router writes both a ctx file (`/tmp/ticket-auto-{ID}-ctx.txt`) and a spawn-meta file (`/tmp/ticket-auto-{ID}-spawn-meta.txt`, stamped with the orchestrator's own `SESSION_ID`). The token-tracker hooks derive TICKET_ID/PHASE/LOG_FILE solely from the spawn-meta file whose `SESSION_ID` matches the firing hook's own `session_id` — no ls -t scan across unrelated sessions, no ctx-file fallback. These files outlive the spawn by design (env.sh is sourced by later phases, spawn-meta by duplicate `spawn_agent_post` calls), so their lifetime is bounded by age: the `SessionStart` hook `hooks/tmp-sweep.sh` removes a ticket's whole scratch group once none of its files has been touched for `TICKET_TMP_TTL_MIN` minutes (default 24h).
- **Post-trigger assertions (retired)**: `flow.sh` no longer mutates the tracker, so there is nothing left to assert against it. Exit code 7 (`STATE_ASSERTION_FAILED`) is retired along with the assertion it reported; every consumer that branched on it has been updated. The board projection is instead observable best-effort, through `META|board-projection|fail` on a permanent driver failure and a `META|board-dead-letter` marker plus operator notification after repeated transient ones (`FLEET_BOARD_MAX_ATTEMPTS`, default 5) — the pipeline's own correctness no longer depends on the tracker agreeing.
- **Idempotency**: `flow.sh` computes the desired `(stage, flags)` from the manifest's current values plus a trigger's `adds`/`removes` (filtered to the four labels a board driver may ever project) and writes the transition — `stage`, `flags`, `rev`, `pending_event` — unconditionally on every invocation, even when the computed values equal the current ones. "No manifest state change" describes the `(stage, flags)` *values* staying the same, not that no write occurs: `rev` always advances by one and the trigger's event always emits with a fresh idempotency key, which is what keeps a structurally nil-effect trigger (e.g. `implement-outcome`, `re-claim` — always `to: null`, no label delta) from being silently dropped from the fact stream the way the old Linear-idempotent-skip path used to drop it.
