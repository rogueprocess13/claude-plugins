...
All Linear API mutations flow through `flow.sh` (the `ticket-flow` skill executor) — a deterministic bash script. Skills never call Linear mutation endpoints directly. `flow.sh` handles: state transitions, label add/remove, assignee changes, idempotency checks, post-trigger state assertions, and generation fence checks (`{tid}-fence` — rejects mutations from superseded generations, gated behind `FLEET_FENCE_ENFORCE=true`). The state machine definition lives in `skills/ticket-flow/workflow.json` (renamed from `state-machine.json` — tracker-event-vocabulary-and-emitter) — `flow.sh` reads it, so changes to the JSON take effect without script changes. The same file also carries a `vocabulary` section (the canonical, board-agnostic event catalog `lib/events.sh`'s `emit_event` validates against — see "Event outbox" below) and a `board_drivers` placeholder reserved for a later phase.

**Event outbox** (tracker-event-vocabulary-and-emitter, Phase B1 of the tracker-decoupling programme): `ticket-auto-pipeline/lib/events.sh` exposes `emit_event <tid> <event> <json>` — the single entry point for writing a fact to a ticket's append-only outbox (`{tid}-outbox.jsonl`, JSON Lines, one record per line: `{seq, tid, ts, gen, event, data, from_hint}`). Independent of `flow.sh` — gate holds and human holds write directly to the pipeline log and never call `flow.sh`, so emission cannot live inside it (`flow.sh`'s own idempotency rule would swallow a hold's emission, which changes no label or state, even if it did). Every event-producing site — `flow.sh`-driven transitions (dual-write, via a generic trigger→event lookup in `workflow.json`'s `vocabulary`), gate holds/releases, human holds/releases, PR open/merge, verify retries, and fleet-initiated kills — calls `emit_event` directly or indirectly. Dual-write only in this phase: nothing consumes the outbox yet, and no call site stops writing to Linear or the pipeline log. See `openspec/changes/tracker-event-vocabulary-and-emitter/` (or its archived spec once promoted) for the full design.
