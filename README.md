...
|--------|---------|--------------|
| [`ticket-auto-pipeline`](ticket-auto-pipeline/README.md) | 0.51.0 | **The core.** Takes a Linear ticket and appraises, implements, verifies, and merges it with zero user input. 20+ slash commands, state-machine flow control, deterministic safety gates. |
...
| [`fleet-controller`](fleet-controller/README.md) | 0.32.0 | **Above the core.** Dispatches planned tickets, monitors every running pipeline via 19 detection engines, kills and restarts stuck runs, feeds results back to the planner. Bash-only. |
