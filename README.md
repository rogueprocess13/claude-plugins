...
|--------|---------|--------------|
| [`ticket-auto-pipeline`](ticket-auto-pipeline/README.md) | 0.50.15 | **The core.** Takes a Linear ticket and appraises, implements, verifies, and merges it with zero user input. 20+ slash commands, state-machine flow control, deterministic safety gates. |
...
| [`fleet-controller`](fleet-controller/README.md) | 0.31.13 | **Above the core.** Dispatches planned tickets, monitors every running pipeline via 15 detection engines, kills and restarts stuck runs, feeds results back to the planner. Bash-only. |
